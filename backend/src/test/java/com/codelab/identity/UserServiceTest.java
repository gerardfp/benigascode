...
class UserServiceTest {
    ...
    @BeforeEach
    void setUp() {
        testUser = new User("test@codelab.local", "encodedPass", "Test User", Role.STUDENT);
        testUser.setId(UUID.randomUUID());
    }

    @Test
    void createUser_Success() {
        when(userRepository.existsByUsername("new@codelab.local")).thenReturn(false);
        when(passwordEncoder.encode("secret123")).thenReturn("encodedSecret");
        when(userRepository.save(any(User.class))).thenReturn(testUser);

        UserDTO result = userService.createUser("new@codelab.local", "secret123", "Test User", Role.STUDENT);
    ...
    void createUser_DuplicateUsername_ThrowsValidationException() {
        when(userRepository.existsByUsername("duplicate@codelab.local")).thenReturn(true);

        assertThrows(ValidationException.class, () ->
            userService.createUser("duplicate@codelab.local", "secret123", "Test User", Role.STUDENT)
        );
