...
Esta guía detalla el aprovisionamiento, configuración y puesta en marcha de CodeLab sobre una instancia **OCI Ampere A1 (ARM64)** dentro de los límites gratuitos permanentes de Oracle Cloud.
...
2. **Puertos estrictamente cerrados al exterior:**
   - Puerto `5432` (PostgreSQL) — solo accesible desde la red Docker interna `codelab-network`.
...
## 4. Despliegue de CodeLab

1. Clonar el repositorio en `/opt/codelab`:
   ```bash
   sudo mkdir -p /opt/codelab
   sudo chown $USER:$USER /opt/codelab
   git clone <URL_REPOSITORIO> /opt/codelab
   cd /opt/codelab
   ```
...
2. Configurar variables de entorno de producción:
   ...
   # PostgreSQL
   DB_NAME=codelab
   DB_USER=codelab_user
   DB_PASSWORD=GENERATE_STRONG_RANDOM_PASSWORD_HERE
   
   # CodeLab Backend
   SPRING_PROFILES_ACTIVE=prod
   RUNNER_TOKEN=GENERATE_RUNNER_SECRET_TOKEN_HERE
   CONTENT_REPO_SSH_KEY_PATH=/etc/codelab/keys/id_ed25519
   
   # Caddy & Dominio
   DOMAIN_NAME=codelab.tudominio.edu.es
...
4. Comprobar salud de los contenedores:
   ...
   curl -I https://codelab.tudominio.edu.es/actuator/health
   ```
...
0 3 * * * /opt/codelab/deployment/scripts/backup.sh >> /var/log/codelab-backup.log 2>&1
```
